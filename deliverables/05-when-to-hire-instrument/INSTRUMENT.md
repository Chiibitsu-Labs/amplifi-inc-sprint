...
**"Independent of `Due`" does NOT mean independent of the rollout epoch,
though ~ apply the `Start ≥ 2026-09-04` eligibility test to the
`accepted`/`revising (reopened)` population FIRST, before running this
completeness check on it (the `cancelled` population validated for the
separate corroborating count is unaffected, that signal is never
epoch-clamped).** A row whose `Start` predates the epoch is already
excluded from gates (a)/(b) entirely regardless of its tags (the
numerator-level epoch gate elsewhere in this section), so a malformed or
undated tag entry on a pre-epoch-ineligible row can never actually swing
either gate ~ hard-blocking FIX_CORPUS to PROVISIONAL over it is a false
block, not the honest caution this gate exists to provide (Codex catch,
2026-07-19: the coded (§5b) version of this same gate had the identical
gap).

**The epoch filter closes the pre-epoch case, but the SAME "could this
defect ever have mattered to THIS month's reading" question applies one
more time, for a DIFFERENT reason: a validly-dated entry that's simply
OLD ~ post-epoch, but its own date falls outside the CURRENT trailing-90-
day window ~ can't swing this month's numerator or tag-share either,
regardless of whether its qualifier is complete.** This is narrower than
the epoch carve-out above, and only covers ONE specific failure shape:
an entry that HAS a real, parseable, in-hand date (so we genuinely KNOW
it's out of window, not just suspect it) but is missing its `(missing)`/
`(not-followed)` qualifier or otherwise incomplete on that ONE entry.
Don't let THAT specific gap hard-block the current walkthrough
indefinitely ~ flag it as a data-hygiene note to fix in the log (it's
still real incomplete data, worth cleaning up), but don't mark FIX_CORPUS
PROVISIONAL over a defect that structurally cannot enter this month's
90-day window no matter when it gets repaired (Codex catch, 2026-07-19:
running full completeness validation "independent of whether any entry's
date would land it in-window" was necessary for UNDATED entries, whose
window membership is genuinely unknown, but got applied just as
unconditionally to entries that already carry a known, valid, confirmably
out-of-window date, hard-blocking every future walkthrough off a defect
that can never change any of their answers). **This narrow carve-out does
NOT extend to an UNDATED entry, a malformed-date entry, or a ROW whose
entry count falls short of `Rounds` (a round with no entry at all,
dated or not) ~ all three stay hard-blocking regardless of window,
exactly as before, since their window membership is genuinely unknown,
not just old.** Only an entry that is fully present with its own
confirmed, valid, out-of-window date gets this treatment; anything whose
date can't be confirmed keeps validating unconditionally.
...
2. **REDESIGN check:** open each client's `delivery-log.md`, two reads ~
   ...
   - **Confirmed (on-cadence):** **validate EVERY date this calculation
     ...
     have swung the result across REDESIGN's currently active on-cadence
     threshold either way, the reading isn't clean to route on (or rule
     out) at all, exclusion or not.** **"Currently active" means whatever
     `router-decisions.md`'s approved threshold state says it is RIGHT
     NOW (§5b's ingestion below reads this), never a value frozen at
     implementation time** ~ this bracket test has to move when Michele
     recalibrates the threshold at quarterly review the same way the
     signal's own pass/fail read already does, or the completeness gate
     ends up validating incomplete data against a threshold that's no
     longer the one actually deciding the route: with an active threshold
     recalibrated to 85% and an honest bracket of 82–88%, testing against
     the stale v1 seed of 80% would report this reading as fully resolved
     (both ends clear 80%) even though the missing row can still flip
     whether REDESIGN fires against the REAL, currently-approved 85% line
     (Codex catch, 2026-07-19: this gate's own worked example below uses
     the v1-seeded 80% throughout, which is correct only as long as 80%
     IS the active threshold, and silently becomes wrong the moment it
     isn't). v1 seeds the active threshold at 80% (same seed the signal's
     own definition above states), so every "80%" in this bracket
     procedure below, including the worked example, means "the active
     threshold, currently 80%" ~ substitute whatever value is actually
     approved at evaluation time. Excluding one malformed LATE row can
     raise the rate enough
     to clear the active threshold and wrongly rule out REDESIGN;
     excluding one malformed
     ON-TIME row can just as easily push a clean rate below the active
     threshold and
     ...
     **bound each excluded row by what its OWN status actually makes
     ...
     produced.** **The `(missing)` case needs the identical care in the
     OPPOSITE direction: ruling REDESIGN out here isn't the same as the
     cadence miss being explained, it's DEFERRING that explanation to
     FIX_CORPUS ~ and the deferral only holds if FIX_CORPUS's OWN gates
     then actually clear on it.** FIX_CORPUS's gate (a), below, tests
     PORTFOLIO-WIDE rounds-per-report, not just these late cycles' own
     rate: a handful of late reports each carrying one confirmed
     `brief-misalign (missing)` round can get pulled out of REDESIGN's
     consideration here while the PORTFOLIO average, diluted by many
     OTHER clean reports, never clears gate (a)'s own ≥2-rounds/report
     line. When that happens, NEITHER route fires ~ REDESIGN correctly
     stayed out because the tags say corpus, not process, but FIX_CORPUS
     never actually confirms it, because the frequency test that decides
     it lives at the portfolio level, not the late-cycle level ~ and HIRE
     would proceed as if no non-capacity explanation exists at all, the
     same silent-fall-through failure the `(not-followed)` fix above
     already closed for the opposite qualifier (Codex catch, 2026-07-19).
     **Majority `(missing)` alone is not sufficient to rule REDESIGN out
     permanently:** it defers the question to FIX_CORPUS, but the cadence
     miss stays counted as UNEXPLAINED, blocking/provisional against HIRE,
     until FIX_CORPUS's gate (a)/(b) sequence (step 3 below) ACTUALLY
     fires on this same evidence. If it doesn't clear (the portfolio-wide
     frequency never reaches the line), the late-cycle cadence failure
     reverts to blocking HIRE the same way an ordinary REDESIGN-eligible
     miss would, rather than vanishing between two routes that each
     correctly ruled themselves out on a technicality.** Averaging tags across every accepted
     ...
     **PROVISIONAL, a distinct third state from "fired" and "genuinely
     ...
     question. **Even for that genuinely narrow, still-PROVISIONAL
     candidate, apply the SAME portfolio-scope/narrow-signal-exclusion
     test §3 uses for a CONFIRMED narrow signal, before letting the
     provisional state block HIRE unconditionally.** A confirmed narrow
     REDESIGN/FIX_CORPUS signal doesn't block HIRE outright ~ §3's
     override lets HIRE through anyway if the roster, EXCLUDING that
     signal's own named analyst(s), independently clears the
     more-than-half-elevated bar on its own. Treating an unresolved
     (PROVISIONAL) version of the identical signal as a HARDER block than
     its own eventual confirmed state would is backwards: the missing
     detail is only ever "which specific corpus-vs-process tag applies,"
     never "whether this signal is narrow," so the same exclusion-and-
     recount already answers whether the REST of the roster's elevation
     survives independently, regardless of how that narrow question
     eventually resolves (Codex catch, 2026-07-19: one unresolved late row
     on a single client could otherwise veto a hire decision that's
     already independently justified by more than half the REST of the
     roster running hot). If that post-exclusion count clears the bar,
     don't block HIRE on the provisional state; if it doesn't, the
     provisional block still stands until the qualifier resolves.
   - **Early warning (cycle time):** compare recent `Delivered − Start`
     ...
     step's answer first ~ Codex catch, 2026-07-18. **"That's what step 3
     is for" is a deferral, not a guarantee ~ the same care the on-cadence
     read above needs applies here too: don't treat a corpus-tagged slow
     cycle as explained just because it was excluded from THIS bullet's
     evidence.** Step 3's FIX_CORPUS gate (a) tests PORTFOLIO-WIDE
     rounds-per-report, not just this client's own slow cycles ~ a handful
     of corpus-tagged slow cycles can get pulled out of early-warning
     REDESIGN evidence here while the portfolio average, diluted by many
     other clean reports, never clears gate (a)'s own line, leaving
     NEITHER route to actually confirm the explanation (Codex catch,
     2026-07-19: the same silent-fall-through gap already fixed for
     on-cadence's majority-`(missing)` case, unmirrored here). If step 3
     doesn't actually fire on this same evidence, this cycle-time reading
     reverts to blocking/provisional against HIRE rather than being
     treated as a settled non-issue.)
...
1. **Ingest feed 2, `learnings/patterns.md`, AND each client's current
   ...
   **`Cadence` needs the identical treatment as `Period`/`Status`, not a
   ...
   `Cadence` value doesn't error, it just never matches ANY cadence/version
   filter and silently drops out of every cohort that reads it ~ the row
   goes on contributing to on-cadence and rework tallies (which don't key
   on `Cadence`) while quietly vanishing from cycle-time's baseline and
   trailing-window averages specifically, with nothing flagging the gap, so
   a real slowdown captured in that row can go uncounted and read as a
   ...
   **Also ingest each client's `brief.md` Snapshot ("Amplifi lead
   ...
   trying to scope (Codex catch, 2026-07-19). **Ingest the Snapshot's
   `Account status` field (`active`/`paused`/`ended`) in this SAME pass
   too, not lead/backup ownership alone** ~ REDESIGN's clustering-breadth
   predicate (below) needs it to derive "the currently-active client
   roster," and it has no other structured source for that count.
   Omitting it here doesn't just leave that one field stale, it leaves the
   breadth predicate unable to run automatically at all, forcing the
   supposedly-coded route back to a manual, human-confirmed input every
   single evaluation (Codex catch, 2026-07-19: this ingestion list named
   lead/backup ownership specifically, even though `Account status` was
   added to this same Snapshot table for exactly this predicate to read,
   and was never added to what actually gets pulled in). **Parse and
   validate this field before trusting it for the breadth denominator ~
   ingesting the raw cell text isn't enough.** `brief.md`'s Snapshot cell
   isn't a bare enum, it's a dated value, `{active/paused/ended}, since
   {YYYY-MM-DD}` ~ extract just the status word from before the comma,
   never match on the cell's full text. Require the extracted value to be
   an EXACT match against one of the three defined states (`active`/
   `paused`/`ended`, case-sensitive, no synonyms) ~ a blank cell (never
   filled), a typo, different casing, or any other malformed value is
   INVALID, not a silent default to either "active" or "excluded." **On
   any real (non-`_template-client`) client with an invalid `Account
   status` value, data-BLOCK the breadth predicate's result entirely for
   this evaluation, don't silently exclude that one client from the
   roster and proceed** ~ silently excluding it can shrink the active
   roster enough to flip which side of the "half of active clients" line
   a genuine slowdown lands on, exactly the kind of quiet denominator
   drift the breadth predicate's own accuracy depends on avoiding (Codex
   catch, 2026-07-19: the ingestion spec named the field to pull but never
   defined how to parse its actual dated-cell shape or what to do with a
   blank/malformed value, leaving both up to whatever an implementation
   happened to guess). Flag the specific client and its bad value, the
   same data-hygiene-gap treatment other malformed structured fields get
   elsewhere in this ingestion, and treat the breadth read as
   PROVISIONAL/not-yet-evaluable until it's fixed, rather than computing
   a denominator that quietly dropped a real client. Keep both fields
   updated
   alongside the log in the same weekly pass ~ `brief.md`'s own
   maintenance-trigger note (per `README.md`) means either one changes on
   its own trigger event (reassignment; a pause/end), not on a fixed
   schedule, so a weekly re-read (not a
   one-time import) is what keeps either from silently drifting stale
   between changes.
   ...
   **Upsert alone can only ever add or update a record ~ it has no way to
   ...
   alone.
   **This key-existence diff alone still leaves one gap: a row whose
   `(client, Period)` key is STILL PRESENT in the source but whose CONTENT
   just turned invalid** (a prior, valid, already-upserted row edited into
   something the completeness-gate validation above now rejects ~
   `accepted` typo'd to `acceptd`, `Period` accidentally blanked, a
   `Cadence` value corrupted mid-edit). The completeness gate correctly
   excludes this row from the upsert, but the key-existence diff above
   only looks for keys that DISAPPEARED from the source entirely ~ this
   row's key is still right there, just attached to now-invalid content,
   so the diff sees "key exists, nothing to delete" and leaves the OLD,
   still-valid destination record (from whenever this row last passed
   validation) sitting in Supabase untouched. The router then keeps
   reading that stale copy's status/rework evidence indefinitely, with
   only a data-hygiene flag raised on the side, not an actual correction
   to what the router sees (Codex catch, 2026-07-19: the reconciliation
   spec deletes on absence, but a row that regresses from valid to
   invalid without ever disappearing from the source file falls through
   both the upsert path, which correctly refuses to write it, and the
   delete-diff, which finds its key unchanged and does nothing). Fix:
   every row the completeness gate rejects THIS run, for a key that DOES
   still exist in the destination from a prior successful upsert, must
   also invalidate or remove that destination record as part of the SAME
   pass ~ don't just skip the upsert and flag a warning, actively clear
   the stale copy too (a soft-delete/invalidation flag the router treats
   as absent works as well as a hard delete, as long as no downstream
   read can still see it as current). Once the row is corrected back to
   valid in the source, the next weekly pass's normal upsert restores it
   ~ this only ever removes a row from live routing data while its source
   is confirmed broken, never a healthy row.
   ...
   **One more required source: `learnings/router-decisions.md`'s own
   recorded threshold snapshots, not just raw delivery-log/patterns/brief
   data.** Everything above ingests the RAW inputs the router computes
   from, but several of the numbers those computations depend on aren't
   pure functions of raw data alone ~ WIP's and cycle-time's baselines can
   be manually seeded or deliberately held (the healthy-seed/quiet-window
   guards above, both explicitly human judgment calls, not code checks),
   and every v1 threshold in this doc (on-cadence's `<80%`, gate (a)'s
   frequency line, WIP's `+2` margin, etc.) can change at Michele's
   quarterly review. A coded router that recomputes these purely from raw
   rows, or that hard-codes whatever values existed at implementation
   time, will disagree with the manual router the moment ANY such
   decision happens ~ two systems both claiming to be "the" router,
   reading different thresholds off the same underlying data, is worse
   than either one alone (Codex catch, 2026-07-19: this ingestion spec
   pulled delivery logs, patterns, and select `brief.md` fields, but never
   the approved threshold/baseline STATE `router-decisions.md` exists to
   record, leaving no canonical source for what the CURRENT, human-
   approved numbers actually are). **Resolve EACH signal INDEPENDENTLY to
   its own most recent POPULATED value across walkthrough entries, never
   "the latest walkthrough's snapshot" taken wholesale.** `router-
   decisions.md`'s own template explicitly allows a signal's line to sit
   blank/`n/a` in a given walkthrough when that route genuinely wasn't
   evaluated that month, WITHOUT dropping the line entirely ~ a coded
   ingestion that just reads the newest dated entry and trusts every line
   in it would overwrite a still-active, currently-in-force threshold or
   frozen baseline with `n/a` the moment ANY walkthrough happens to skip
   evaluating that one signal, making the automated branch unevaluable on
   a signal that never actually changed, or forcing a silent fallback to
   a recomputed/hard-coded value that then disagrees with the manual
   ...
   log by hand already would. Ingest the frozen baseline values and their
   freeze/recalibration dates, the
   cadence version each baseline is keyed to (§2's cadence-version-
   matching rule), and every current threshold Michele has adjusted away
   from its v1 seed, each resolved this same per-signal way. Treat each
   resolved value as authoritative over anything
   the code would otherwise derive on its own; a walkthrough's manually
   recorded decision is the source of truth for what's currently in
   force, the same way this whole document already treats Michele's
   quarterly adjustments as overriding the v1 seed values everywhere
   else.
...
**Pin this to the literal ISO date `2026-09-04`, never a bare `Sep 4`
   ...
   **filtered to rounds whose OWN date falls in the CURRENT trailing-90-
   day window ~ the exact same window gate (a)/(b) use for their round-
   level filter, just without the rollout-epoch clamp (this signal is
   deliberately NOT epoch-clamped, unlike gate (a)/(b)'s core math ~ see
   above).** Without this window filter, a cancellation's dated rounds
   stay part of the corroborating tally FOREVER, however old ~ a
   cancellation from years back can surface as if it were current support
   for THIS month's FIX_CORPUS reading, even when no recent cancelled work
   shows the same pattern, distorting a reading that's supposed to be
   "does this month's evidence corroborate," not "has this ever happened"
   (Codex catch, 2026-07-19: this tally had no date-window filter at all,
   only the qualifier split, so it could never age out stale corroborating
   evidence the way the core gates already do for their own rounds).
   Recompute this window fresh every walkthrough, same as gate (a)/(b)'s
   own trailing-90-day window, so a cancellation ages out of the
   corroborating count exactly when its rounds would have aged out of
   gate (a)/(b)'s math had the row not been cancelled.
   ...
   **REDESIGN and HIRE
   ...
   - rework rounds/report meeting or exceeding threshold FIRST (≥, not a
     ...
     - **Completeness gate, run FIRST, before any cohort/`Due`/window
       filtering
       ~ against every `accepted` OR `revising (reopened)` row with
       `Rounds ≥ 1`, PLUS every `cancelled` row with `Rounds ≥ 1` for the
       ...
       to begin with).** **"Before any cohort/window filtering" has ONE
       carve-out, though, and only for the `accepted`/`revising
       (reopened)` population that feeds gates (a)/(b): apply the
       rollout-epoch `Start ≥ 2026-09-04` eligibility test to THAT
       population BEFORE running completeness validation on it, not
       after.** A row whose `Start` predates the epoch is ALREADY excluded
       from gates (a)/(b) entirely, regardless of whether its tags are
       well-formed (the numerator-level epoch gate elsewhere in this
       section) ~ a malformed or legacy tag entry on a pre-epoch-ineligible
       row can never actually swing either gate, so running completeness
       validation on it anyway and hard-blocking FIX_CORPUS to
       PROVISIONAL over data that could never have mattered either way is
       a false block, not caution (Codex catch, 2026-07-19: this gate ran
       "before any cohort/window filtering," which read as including the
       epoch filter too, when the epoch eligibility test needs to apply
       FIRST specifically because it changes which rows even need
       validating, unlike the `Due`-window filter this gate is correctly
       positioned ahead of). **This epoch pre-filter does NOT apply to the
       `cancelled`-row population validated here for the SEPARATE
       corroborating count** ~ that count is never epoch-clamped (see
       above, it surfaces alongside FIX_CORPUS but stays outside gates
       (a)/(b) entirely), so every `cancelled` row with `Rounds ≥ 1` still
       needs full completeness validation regardless of its `Start`.
       **One more narrow carve-out, same reasoning as the epoch one above
       but for a different failure shape: a validly-dated entry whose OWN
       date confirmably falls outside the CURRENT trailing-90-day window
       (post-epoch, just old) can't swing this month's numerator or
       tag-share either way, so a missing qualifier on THAT ONE entry
       specifically shouldn't hard-block this walkthrough ~ flag it as a
       data-hygiene note instead (Codex catch, 2026-07-19: this gate
       validated every entry "independent of whether any entry's date
       would land it in-window," correct for an UNDATED entry whose
       window membership is unknown, but applied the same hard-block to
       an entry that already carries a known, valid, out-of-window date
       and so can never affect the current reading regardless of repair).
       This does NOT extend to an undated entry, a malformed-date entry,
       or a row whose entry count falls short of `Rounds` (an entirely
       missing round, dated or not) ~ all three keep hard-blocking
       regardless of window, since their window membership is genuinely
       unknown, not just old.**
       Scoping THIS gate to `accepted`/`revising
   ...
   - on-cadence rate below threshold, OR cycle time trending up against
     ...
     **read from `accepted` OR `revising (reopened)` rows of those cycles
     ...
     the first one:**
     - **Computable (breadth), from the SAME ingested `Account status`
       field §5b's brief-ingestion step (below) already pulls in ~ derive
       the denominator from it directly, never a manual fallback:** the
       number of currently-active clients (`Account status = active`)
       individually showing a
       REDESIGN-triggering signal THIS evaluation cycle (on-cadence
       failing its bracketed read against the CURRENTLY ACTIVE on-cadence
       threshold ~ read from `router-decisions.md`'s approved state, same
       ingested value the signal's own pass/fail test above uses, never a
       hard-coded 80%, so this breadth predicate stays in sync with the
       signal it's counting instead of drifting to a stale v1 seed the
       moment Michele recalibrates it (Codex catch, 2026-07-19: this
       predicate re-stated the literal `<80%` figure instead of reading
       the same ingested threshold state the signal check itself already
       depends on) ~ OR cycle-time trending up, per the
       ...
       half of "clustered"; half or more is "broad." **`cancelled` (a
       per-CYCLE status on individual `delivery-log.md` rows) still can't
       answer "is THIS client's engagement itself still active" ~ that's
       exactly why `brief.md`'s Snapshot carries its OWN, separate
       `Account status: active/paused/ended` field, and why §5b's
       brief-ingestion step pulls it in alongside lead/backup ownership**
       (Codex catch, 2026-07-19: this predicate still described itself as
       blocked on a field that doesn't exist and a count that has to stay
       manual, even though `brief.md`'s Snapshot already defines
       `Account status` and the brief-ingestion item below already
       ingests it ~ an implementer reading this bullet in isolation could
       leave the predicate on a manual input, or implement something that
       disagrees with the ingestion spec, on data this doc already has).
       Exclude any `paused`/`ended` client from the denominator entirely,
       not counted as a non-triggering (healthy) member of it ~ the two
       read very differently for this predicate (excluding shrinks the
       denominator honestly; miscounting as healthy inflates it with an
       account that was never available to trigger anything). Code CAN
       derive and hard-block on this count directly from the ingested
       field, the
       same class of predicate `more than half of roster` already is
       everywhere else in this doc, no manual confirmation step needed
       once `Account status` is actually populated for every client
       (`ROADMAP.md` task 1.3b requires setting it at onboarding).
   ...
   - HIRE requires ALL of: sustained load over the structural line AND
     ...
     **read the CURRENT owner from `brief.md`'s Snapshot table ("Amplifi
     ...
     someone happens to ship); it's the ONLY source for PERMANENT
     ownership. **But the exclusion SET for this test isn't ownership
     alone ~ it's whoever is ACTUALLY carrying this client's capchecker
     load right now, which can genuinely be the backup, not the lead,
     during an active temporary cover** (the lead on leave, say, with the
     backup shipping this client's recent cycles in the meantime):
     `brief.md`'s lead field alone would exclude the lead (correctly not
     elevated, since they're not currently doing this client's work) while
