...
1. **Ingest feed 2, `learnings/patterns.md`, AND each client's current
   ...
   **One more required source: `learnings/router-decisions.md`'s own
   ...
   approved numbers actually are). Ingest `router-decisions.md`'s most
   recent threshold snapshot for each signal in this SAME weekly pass ~
   the frozen baseline values and their freeze/recalibration dates, the
   ...
   from its v1 seed. Treat this snapshot as authoritative over anything
