...
| 1.4a | Set up Drive for Desktop synced to `amplifi-knowledge/`, AND install `amplifi-improve` separately in that write-capable client (Claude Code or equivalent) ~ **required before 1.4 can be called done**, not optional follow-up. The Claude Enterprise install alone leaves the capture loop unable to save anything (`DRIVE-HANDOFF.md` steps 6a/6b); without this, Phase 1 could look complete while zero learning files are ever written and the first promotion pass (1.6) has nothing to process | **Chii** | Aug 7, same day as 1.4 | capture loop silently does nothing |
